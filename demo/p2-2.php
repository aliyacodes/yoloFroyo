...
function showForm(){# shows form so user can enter their name.  Initial scenario
	...
	foreach ($config->items as $item){//Loop through Item Objects - getting id, name and description.
            echo '<p><b>Qty.</b>
		  ...
		  <option value="4">4</option>
		   ...
		   foreach ($item->Extras as $key => $toppings){//Loop through topping options of each Item object.
	 ...
	 $toppingTotal = '';
         ...
         foreach ($_POST as $name => $value){//loop the form elements
          // $value is the Qty of each item.
		    ...
		    //if form name attribute starts with 'item_', process it
		    if (substr($name,0,7)=='extra_1'){//if it's a topping and starts with 'extra_1'
		         if ((int)$value > 0 ){
